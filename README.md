...
## Update
* **June 10, 2026** We are testing a new license tool, where lib_cxx.so and license_checker are removed and Python 3.14 is supported. There are slight changes in API and feature id generation. **The current version will be replaced in one or two months.** We are also planning to add more features and support aarch64 in the future. Help us test the new version if you are interested! See the [dev](https://github.com/graspnet/anygrasp_sdk/tree/dev) branch for the update.
